...
model.nn<-train(bikes~short_profile_3h_diff_bikes+short_profile_bikes+bikes_3h_ago,data=train_9,trControl=train_control,method="knn",na.rm=T) 
print(model.nn)
...
#prediccion
prediccion<-predict(model.nn,df_deploy_8)
prediccion
...
#test
test<-round(predict(model.nn,df_test))
test[test<0]<-0
test[test>max]<-max
...
write.csv(resultado,file='example_submission3.csv')
